...
| **Differential** | **0 divergences** in 6,272 fixed-corpus + 3,816 JSONTestSuite comparisons, across all 4 input sources ([matrix](docs/differential-sources.md)) and 28 drive modes |
...
| C-20 | No divergence has ever been observed on any input where the pinned original is well defined: 6,272 fixed-corpus comparisons plus 3,816 JSONTestSuite comparisons plus 11,720,000 published fuzz cases -- 11,730,088 in total, all at zero. | verified | [`artifacts/verification-report.json`](artifacts/verification-report.json) |
...
| C-25 | An implementation-independent invariant checker validates every transcript both implementations produce against 13 rules that reference neither of them, with zero violations on either side -- 5,824 transcripts and 203,433 records from the committed corpus alone, and more when the optional standards corpus is present. | verified | [`artifacts/invariants/summary.json`](artifacts/invariants/summary.json) |
...
| C-34 | The Zig port costs a consumer more space than the original: linking one identical C program against the Zig archive rather than the pinned original's object grows both the stripped executable and the machine-code section, by a factor the artifact records for the host it was measured on. | verified | [`artifacts/size-report.json`](artifacts/size-report.json) |
