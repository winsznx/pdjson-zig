...
| C-04 | Across 5,805 differential comparisons on the fixed corpus, spanning all four documented input sources and every exported function, the Zig implementation and the pinned C original produce byte-identical behaviour transcripts, with 0 divergences. | verified | [`artifacts/differential-summary.json`](artifacts/differential-summary.json) |
| C-05 | The differential comparison covers 27 drive modes: four input sources crossed with peek, skip, reset, the separator API and strict mode, plus five json_skip_until targets and four allocation-failure schedules. | verified | [`artifacts/differential-summary.json`](artifacts/differential-summary.json) |
